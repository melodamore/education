...
import { Zap, CheckCircle2, XCircle, ArrowLeftRight, X } from 'lucide-react';
...
export default function TextbookReader({ chapterData }: TextbookReaderProps) {
  ...
  return (
    <div className="relative pb-24">
      ...
      <div className="flex bg-white/50 backdrop-blur-xl p-1.5 rounded-[1rem] w-full mb-8 shadow-inner border border-white/60">
        {(['en', 'am', 'om'] as Locale[]).map((lang) => (
          <button
            key={lang} onClick={() => handleLocaleChange(lang)}
            className={`flex-1 py-2 text-[11px] font-black tracking-widest rounded-lg transition-all ${locale === lang ? 'bg-white text-indigo-600 shadow-sm scale-[1.02]' : 'text-gray-500 active:bg-white/30'}`}
          >
        ...
        <div key={section.id} className="space-y-8">
          ...
          {section.eueeQuestions.length > 0 && (
            <motion.button 
              ...
              onClick={() => { setShowExamMode(showExamMode === section.id ? null : section.id); setSelectedAnswer(null); setIsAnswerSubmitted(false); }} 
              className="w-full flex items-center justify-center space-x-2 bg-indigo-600 text-white font-black py-4 rounded-[1.5rem] shadow-[0_8px_20px_rgba(79,70,229,0.3)] border border-indigo-500"
            >
          ...
          <AnimatePresence>
            {showExamMode === section.id && (
              <motion.div 
                initial={{ opacity: 0, height: 0, scale: 0.95 }} animate={{ opacity: 1, height: 'auto', scale: 1 }} exit={{ opacity: 0, height: 0, scale: 0.95 }} transition={snapPhysics}
                className="overflow-hidden"
              >
                <div className="p-6 bg-gray-900 border border-gray-800 rounded-[2rem] shadow-2xl relative">
                  {/* Internal Exam UI - Dark Mode for Contrast */}
                    ...
                    <div key={qIdx} className="relative">
                      ...
                      <div className="space-y-3">
                        {q.options.map((option) => {
                          ...
                          return (
                            <motion.button 
                              whileTap={{ scale: 0.97 }} transition={snapPhysics}
                              key={option} disabled={isAnswerSubmitted} onClick={() => setSelectedAnswer(optLetter)} 
                              className={`w-full text-left p-4 rounded-[1rem] border-2 font-bold transition-all ${isSelected ? 'bg-indigo-600 border-indigo-500 text-white shadow-[0_0_15px_rgba(79,70,229,0.4)]' : 'bg-gray-800 border-gray-700 text-gray-300 hover:border-gray-600'}`}
                            >
                              {option}
                            </motion.button>
                          );
                      ...
                      {selectedAnswer && (
                        <div className="mt-6">
                          {!isAnswerSubmitted ? (
                            <motion.button whileTap={{ scale: 0.96 }} onClick={() => setIsAnswerSubmitted(true)} className="w-full bg-white text-gray-900 font-black py-4 rounded-[1rem] uppercase tracking-widest">Submit</motion.button>
                          ) : (
      ...
      <AnimatePresence>
        {activeKeyword && (
          <div className="fixed inset-0 z-50 flex flex-col justify-end pointer-events-none">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-gray-900/60 backdrop-blur-md pointer-events-auto" onClick={() => setActiveKeyword(null)}></motion.div>
            
            <motion.div 
              initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={snapPhysics}
              drag="y" dragConstraints={{ top: 0, bottom: 0 }} dragElastic={0.1} onDragEnd={(_, info) => { if (info.offset.y > 100) setActiveKeyword(null); }}
              className="relative bg-white w-full rounded-t-[2.5rem] shadow-[0_-20px_40px_rgba(0,0,0,0.3)] pb-safe pt-2 px-6 h-[85vh] flex flex-col pointer-events-auto sm:max-w-md sm:mx-auto"
            >
              <div className="w-16 h-2 bg-gray-200 rounded-full mx-auto my-4"></div>
              
              <div className="flex justify-between items-center mb-6">
                <h4 className="font-black text-2xl text-gray-900 uppercase tracking-tighter flex items-center">
                   Dictionary
                </h4>
              ...
              <div className="flex-1 overflow-y-auto pb-8 space-y-6 no-scrollbar">
                ...
                <div className="flex space-x-2 w-full">
                  ...
                  <button onClick={() => setVernacular('om')} className={`flex-1 py-3 text-xs font-black uppercase tracking-widest rounded-xl transition-all border-2 ${vernacular === 'om' ? 'border-indigo-600 text-indigo-600 bg-indigo-50/50' : 'border-gray-100 text-gray-400 bg-transparent'}`}>Oromoo</button>
                </div>

                <div className="py-4">
                  <h5 className="text-4xl font-black text-gray-900 capitalize mb-1 tracking-tight">{activeKeyword.word}</h5>
                  <p className="text-2xl font-black text-indigo-600 tracking-tight">{activeKeyword.translations[vernacular]}</p>
                </div>

                <div>
                  <div className="relative w-full h-64 bg-gray-900 rounded-[2rem] overflow-hidden shadow-[inset_0_2px_15px_rgba(0,0,0,0.5)] border border-gray-800">
